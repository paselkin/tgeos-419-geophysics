...
// Internal chip select pin for SPI communication with SD card
const int chipSelect = 4;

// Pins for button and internal LED
const int LED_PIN = 8;
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("GPS Mag Logger v3.0");
  ...
  //SD Card code - checking if SD card is available
...
void loop() // run over and over again
{
  ...
  if (millis() - timer > nextInterval) {
    ...
    dataString = String(yy, DEC) + "-" + String(mm, DEC) + "-" + String(d, DEC) + "\t";
    dataString += String(h, DEC) + ":" + String(m, DEC) + ":" + String(s, 3) + "\t";
    if (GPS.fix) {
      ...
      dataString += String(GPS.altitude) + "\t" + String((int)GPS.satellites) + " Sat \t";
        for (int i = 0; i <= (int)GPS.satellites; i++) {
    digitalWrite(LED_PIN, HIGH);   // turn the LED on (HIGH is the voltage level)
...
#endif
    // read adc data here too
    results = ads.readADC_Differential_0_1();
    dataString += "\t" + String(results * 188) + " uV";
